...
&nbsp;
  if (<span class="missing-if-branch" title="else path not taken" >E</span>entry.registrationStatus === RequestStatus.NOT_STARTED) {
    // The request timed out or failed in a different call. Try again.
    const <span class="cstat-no" title="statement not covered" >{</span>
      installationEntry,
      registrationPromise
    } = <span class="cstat-no" title="statement not covered" >await getInstallationEntry(appConfig);</span>
&nbsp;
<span class="cstat-no" title="statement not covered" >    if (registrationPromise) {</span>
<span class="cstat-no" title="statement not covered" >      return registrationPromise;</span>
    } else {
